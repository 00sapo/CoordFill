...
class CoordFill(nn.Module):
    def __init__(
        self, args, name, mask_prediction=False, attffc=False, scale_injection=False
    ):
    ...
    def forward(self, inp):
        ...
        lr_features = self.asap.lowres_stream(
            self.refine, torch.cat([lr_hole, lr_mask], dim=1), hr_hole
        )
    ...
    def mask_predict(self, inp):
        ...
        lr_features, temp_mask = self.asap.lowres_stream.mask_predict(
            self.refine, torch.cat([lr_hole, lr_mask], dim=1), hr_hole, mask
        )

        output = self.asap.highres_stream.mask_predict(
            hr_hole, lr_features, mask, temp_mask
        )
        output = output * (1 - mask) + hr_hole
