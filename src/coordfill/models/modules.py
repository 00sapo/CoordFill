...
class CoordFillGenerator(BaseNetwork):
    ...
    def __init__(self, opt, hr_stream=None, lr_stream=None, fast=False):
        ...
        self.highres_stream = PixelQueryNet(
            self.downsampling,
            num_inputs=self.num_inputs,
            num_outputs=opt.output_nc,
            width=opt.hr_width,
            depth=opt.hr_depth,
            no_one_hot=opt.no_one_hot,
            lr_instance=opt.lr_instance,
            **hr_stream,
        )
...
class ParaGenNet(th.nn.Module):
    ...
    def __init__(self, num_out, scale_injection=False):
        ...
        if self.scale_injection:
            self.out_para = nn.Sequential(th.nn.Linear(ngf * 8 + 1, self.num_out))
        else:
            self.out_para = nn.Sequential(th.nn.Linear(ngf * 8, self.num_out))
    ...
    def mask_predict(self, model, x, x_hr, mask):
        ...
        mask = mask.permute(0, 2, 3, 4, 5, 1).contiguous().view(bs, h, w, int(k * k))
...
class PixelQueryNet(th.nn.Module):
    ...
    def forward(self, highres, lr_params):
        ...
        tiles = (
            tiles.permute(0, 2, 3, 4, 5, 1)
            .contiguous()
            .view(bs, h_lr, w_lr, int(k * k), nci)
        )
        out = tiles
        ...
        out = out.view(bs, h_lr, w_lr, k, k, self.num_outputs).permute(0, 5, 1, 3, 2, 4)
    ...
    def mask_predict(self, highres, lr_params, hr_mask, lr_mask):
        ...
        highres = (
            highres.permute(0, 2, 3, 4, 5, 1)
            .contiguous()
            .view(bs, h_lr, w_lr, int(k * k), 3)
            .view(bs, h_lr * w_lr, int(k * k), 3)
        )
